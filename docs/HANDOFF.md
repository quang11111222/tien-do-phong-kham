...
- 2026-09-16 đang thực hiện: sửa thông báo khi gửi hoàn thành thiếu bằng chứng/lý do trễ và đưa thay đổi Gantt do công cụ API key để lại vào cùng một bản deploy. Thay đổi gồm `GanttView.tsx`, `workItemDisplay.ts`/test, `prototype.css` và bàn giao; nút gửi không còn khóa âm thầm khi thiếu dữ liệu, thông báo nêu rõ điều kiện còn thiếu. Lint, type-check, 67 tests, build đạt; build vẫn cảnh báo chunk lớn. Browser local xác nhận nhãn Gantt **THỰC TẾ**, tạo riêng công việc `XXII.5.1` trong `PK-DEMO-QA-21` với hạn 15/09/2026 và thấy thông báo chính xác khi bấm gửi mà chưa có tệp; giữ lại công việc QA này. Nhánh thiếu lý do trễ có unit test nhưng chưa browser-test vì công cụ trình duyệt không cho chọn file để tải lên; chưa nộp bằng chứng hoặc thay trạng thái công việc. Chưa commit/push/deploy. Không ghi dữ liệu ở Khe Tre/Sơn Tây. `.gitignore` chỉ có khác biệt line ending và `outputs/` là dữ liệu QA ngoài phạm vi, không stage.
