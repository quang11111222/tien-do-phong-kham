...
- 2026-09-16 Codex đã triển khai local ba yêu cầu UI: cột **Kết thúc thực tế** trên bảng Gantt; tab **Đề xuất việc con** trong panel chỉ cho Nhân viên/Quản trị phòng; tổng số hai hàng đợi bên cạnh menu **Xét duyệt** dùng cùng bộ lọc quyền với màn xét duyệt. Không đổi database/migration/dữ liệu. `type-check`, lint, 64 tests và build đạt; build còn cảnh báo chunk lớn. Browser local bằng phiên Nhân viên trên QA-21 xác nhận cột mới có `16/9/2026` ở việc đã hoàn thành, `—` ở dòng chưa có ngày/hạng mục và tab đề xuất chọn sẵn đúng công việc cha; không gửi đề xuất, không ghi dữ liệu. Chưa commit/push/deploy; chưa kiểm tra UI bằng phiên Quản trị phòng/dự án/hệ thống. `.gitignore` và `outputs/` vẫn là thay đổi có sẵn ngoài phạm vi, không stage.
