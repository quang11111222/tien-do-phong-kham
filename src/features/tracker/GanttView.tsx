...
import { canAddChildWorkItem, canCompleteWorkItemDirectly, canEditWorkItem, canManageWorkItemStructure, canReviewCompletion, canViewWorkItemDetails, participantManagementScope } from '../../lib/permissions'
import { actualCompletionDate, completionSubmissionError, displayWorkItemWbs, requiresLateReason } from './workItemDisplay'
import { useToast } from '../../components/toastContext'
import { WorkProposalForm, WorkProposals } from './WorkProposals'
import { canUseEmbeddedProposalTab } from './workProposalService'
...
type DrawerTab = 'info' | 'evidence' | 'log' | 'proposal' | 'approval'
...
export function GanttView({ project, profile, initialWorkItemId, onSelectedWorkItemChange, onBack, onDirtyChange, onUnreadCountChange }: { project: Project; profile: Profile; initialWorkItemId: string | null; onSelectedWorkItemChange: (workItemId: string | null) => void; onBack: () => void; onDirtyChange: (dirty: boolean) => void; onUnreadCountChange: (count: number) => void }) {
  ...
  useAutoRefresh(() => load().catch(() => setError('Không tự cập nhật được dữ liệu mới.')), { enabled: !drawerDirty && !proposalDirty && !showImport && !showClone })
  useEffect(() => {
  ...
  const endDate = maxDate([project.end_date, ...allLeaves.map((item) => item.end_date), ...allLeaves.map((item) => actualCompletionDate(item.actual_completed_at)), ...milestones.map((item) => item.due_date)]) || startDate
  ...
  const run = async (action: () => Promise<void>, successMessage: string) => { setError(null); try { await action(); await load(); notify(successMessage) } catch (caught) { const message = caught instanceof Error ? caught.message : 'Không thực hiện được thao tác.'; setError(message); notify(message, 'error') } }
  ...
  const openClone = async () => { try { const projects = (await getProjects()).filter((item) => item.id !== project.id); const viable = (await Promise.all(projects.map(async (item) => ({ item, hasPlan: (await getWorkItems(item.id)).length > 0 })))).filter((entry) => entry.hasPlan).map((entry) => entry.item); setCloneSources(viable); setCloneSourceId(viable[0]?.id ?? ''); setShowClone(true) } catch { const message = 'Không tải được danh sách dự án để nhân bản.'; setError(message); notify(message, 'error') } }
  const reportDirty = useCallback((dirty: boolean) => { setDrawerDirty(dirty); onDirtyChange(dirty || proposalDirty) }, [onDirtyChange, proposalDirty])
  const reportProposalDirty = useCallback((dirty: boolean) => { setProposalDirty(dirty); onDirtyChange(dirty || drawerDirty) }, [onDirtyChange, drawerDirty])
  ...
  return <>
    ...
    {loading ? <div className="card empty">Đang tải tiến độ…</div> : <div className="tt"><div className="trow thead"><div className="lft"><div className="cell c-wbs">Mã</div><div className="cell c-name">Hạng mục công việc</div><div className="cell c-lead">Chủ trì</div><div className="cell c-d">Bắt đầu</div><div className="cell c-d">Kết thúc</div><div className="cell c-d">Thực tế</div><div className="cell c-n">Số ngày</div><div className="cell c-st">Trạng thái</div><div className="cell c-act" /></div><TimelineHeader start={startDate} end={endDate} width={width} pixelsPerDay={pixelsPerDay} zoom={zoom} milestones={milestones} /></div>{flatRows.map((item) => {
      ...
      const actualCompletedDate = actualCompletionDate(item.actual_completed_at)
...
function WorkDrawer({ item, items, users, departments, profile, projectCanManage, canViewDetails, canManageStructure, canAddChild, isNew, tab, onTabChange, onClose, onCreated, onAddChild, onChanged, onError, onDirtyChange }: { item: WorkItem; items: WorkItem[]; users: UserProfile[]; departments: Department[]; profile: Profile; projectCanManage: boolean; canViewDetails: boolean; canManageStructure: boolean; canAddChild: boolean; isNew: boolean; tab: DrawerTab; onTabChange: (tab: DrawerTab) => void; onClose: () => void; onCreated: (id: string) => Promise<void>; onAddChild: (parent: WorkItem) => void; onChanged: () => Promise<void>; onError: (message: string | null) => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  const isOverdue = !isNew && !isGroup && requiresLateReason(item.end_date, item.status, today())
  const canEditInfo = canManageStructure
  const participantScope = participantManagementScope({
    role: profile.role,
    canManageProject: projectCanManage,
    departmentId: profile.department_id,
    isDepartmentAdmin: profile.is_department_admin,
    leadDepartmentId: item.lead_department_id,
    coordinatingDepartmentIds: item.coordinating_department_ids,
  })
  const canManageParticipants = !isNew && !isGroup && participantScope !== 'none'
  const canManageEvidence = canUpdate && !isEvidenceLocked
  const canLogProgress = canUpdate
  const canProposeChild = canViewDetails && !isNew && canUseEmbeddedProposalTab(profile.role, projectCanManage)
  ...
  const participantUsersForManager = participantScope === 'own_department'
    ? participantUsers.filter((user) => user.department_id === profile.department_id)
    : participantUsers
  ...
  const hasUnsavedChanges = dirty || proposalDirty
  useEffect(() => {
  ...
  const run = async (action: () => Promise<void>, successMessage: string, after?: () => void) => { setBusy(true); onError(null); try { await action(); await onChanged(); after?.(); notify(successMessage) } catch (caught) { const message = caught instanceof Error ? caught.message : 'Không thực hiện được thao tác.'; onError(message); notify(message, 'error') } finally { setBusy(false) } }
  const requestClose = async () => { if (hasUnsavedChanges && !await confirm({ title: 'Bỏ thay đổi chưa lưu?', message: isNew ? 'Hạng mục/công việc mới chưa được lưu. Đóng panel sẽ bỏ bản nháp này.' : 'Panel đang có thay đổi chưa lưu. Đóng panel sẽ bỏ các thay đổi.', confirmLabel: 'Đóng và bỏ', tone: 'danger' })) return; onClose() }
  const switchTab = async (next: DrawerTab) => { if (!canViewDetails && next !== 'info') return; if (next === tab) return; if (hasUnsavedChanges && !await confirm({ title: 'Chuyển tab?', message: 'Các thay đổi chưa lưu trong panel sẽ bị bỏ.', confirmLabel: 'Chuyển tab', tone: 'danger' })) return; onTabChange(next) }
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!form.name.trim()) { const message = 'Tên hạng mục/công việc không được để trống.'; onError(message); notify(message, 'error'); return }
    if (form.startDate && form.endDate && form.endDate < form.startDate) { const message = 'Ngày kết thúc phải từ ngày bắt đầu trở đi.'; onError(message); notify(message, 'error'); return }
    setBusy(true); onError(null)
    try {
      if (isNew) {
        const id = await createWorkItem({ projectId: item.project_id, parentId: item.parent_id, wbs: item.wbs, ...form })
        await onCreated(id)
        notify(`Đã tạo ${isGroup ? 'hạng mục' : 'công việc'} mới.`)
      } else {
        await saveWorkItem(item, form)
        await onChanged()
        onClose()
        notify(`Đã lưu thay đổi ${isGroup ? 'hạng mục' : 'công việc'}.`)
      }
    } catch (caught) { const message = caught instanceof Error ? caught.message : 'Không lưu được hạng mục/công việc.'; onError(message); notify(message, 'error') } finally { setBusy(false) }
  }
  const saveParticipants = async () => {
    await run(() => saveWorkItemParticipants(item, form.participantIds), 'Đã lưu phân công người tham gia.', onClose)
  }
  ...
  const deleteCurrent = async () => { if (await confirm({ title: `Xóa ${isGroup ? 'hạng mục' : 'công việc'}?`, message: `${item.name}${hasChildren ? ' và toàn bộ công việc con bên trong' : ''} sẽ bị xóa. Không thể xóa nếu bên trong có bằng chứng đã nộp.`, confirmLabel: 'Xóa', tone: 'danger' })) await run(() => removeWorkItem(item.id), `Đã xóa ${isGroup ? 'hạng mục' : 'công việc'}.`, onClose) }
  const deleteEvidence = async () => { if (!canManageEvidence) { const message = 'Bằng chứng đã nộp được khóa và không thể xóa.'; onError(message); notify(message, 'error'); return } if (item.attachment && await confirm({ title: 'Xóa tệp bằng chứng?', message: `${item.attachment.file_name} sẽ bị xóa khỏi công việc. Thao tác này không thể hoàn tác.`, confirmLabel: 'Xóa tệp', tone: 'danger' })) await run(() => removeEvidence(item.attachment!), 'Đã xóa tệp bằng chứng.') }
  const decide = async (decision: 'approved' | 'rejected') => {
    if (!pendingRequest) return
    if (pendingRequest.submitted_by === profile.id) { const message = 'Người gửi không được tự duyệt yêu cầu của mình.'; onError(message); notify(message, 'error'); return }
    if (decision === 'rejected' && !reviewNote.trim()) { const message = 'Từ chối phải nhập lý do.'; onError(message); notify(message, 'error'); return }
    await run(() => reviewRequest(pendingRequest.id, decision, reviewNote), decision === 'approved' ? 'Đã duyệt hoàn thành công việc.' : 'Đã từ chối yêu cầu hoàn thành.', () => onTabChange('evidence'))
  }
  const submitCompletion = async () => {
    const message = completionSubmissionError(Boolean(item.attachment), isOverdue, lateReason)
    if (message) { onError(message); notify(message, 'error'); return }
    await run(() => requestCompletion(item.id, note, lateReason), completesDirectly ? 'Đã nộp bằng chứng và hoàn thành công việc.' : 'Đã gửi công việc cho quản trị viên duyệt.')
  }
  return <><aside className="drawer on" aria-label={isNew ? `Thêm ${isGroup ? 'hạng mục' : 'công việc'}` : 'Chi tiết công việc'}><div id="dwrap"><div className="dhead"><div style={{ flex: 1 }}><div className="eyebrow">{isGroup ? 'Hạng mục' : 'Công việc'} · {item.wbs}</div><h2>{isNew ? `Thêm ${isGroup ? 'hạng mục' : 'công việc'} mới` : item.name}</h2>{isNew ? <span className="tiny unsaved">Chưa lưu — chỉ nút Lưu mới ghi vào tiến độ</span> : <span className={`pill ${classes[displayedStatus]}`}><i />{displayedStatus === 'late' ? 'Quá hạn' : labels[displayedStatus]}</span>}</div><button className="btn" aria-label="Đóng panel" onClick={() => void requestClose()}>✕</button></div><div className="dtabs"><button className={tab === 'info' ? 'on' : ''} onClick={() => void switchTab('info')}>Thông tin</button>{canViewDetails && !isNew && (!isGroup || item.attachment) && <button className={tab === 'evidence' ? 'on' : ''} onClick={() => void switchTab('evidence')}>Bằng chứng <span className="cnt">{item.attachment ? 1 : 0}</span></button>}{canViewDetails && !isNew && <button className={tab === 'log' ? 'on' : ''} onClick={() => void switchTab('log')}>Diễn biến <span className="cnt">{updates.length}</span></button>}{canProposeChild && <button className={tab === 'proposal' ? 'on' : ''} onClick={() => void switchTab('proposal')}>Đề xuất việc con</button>}{canViewDetails && !isNew && canReviewItem && item.status === 'pending_approval' && <button className={`approval-tab ${tab === 'approval' ? 'on' : ''}`} onClick={() => void switchTab('approval')}>Xét duyệt <span className="cnt">1</span></button>}</div><div className="dbody">
    ...
    {tab === 'info' && <><form className="sec" onSubmit={(event) => void submit(event)}><h3>Thông tin {isGroup ? 'hạng mục' : 'công việc'}</h3><div className="fg"><label className="f wide"><span>{isGroup ? 'Tên hạng mục' : 'Tên công việc'}</span><textarea autoFocus={isNew} disabled={!canEditInfo} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label className="f wide"><span>Đơn vị chủ trì</span><select disabled={!canEditInfo} value={form.leadDepartmentId} onChange={(event) => { const leadDepartmentId = event.target.value; changeDepartments(leadDepartmentId, form.coordinatingDepartmentIds.filter((id) => id !== leadDepartmentId)) }}><option value="">Chưa chọn đơn vị chủ trì</option>{departments.map((department) => <option key={department.id} value={department.id}>{department.code} — {department.name}</option>)}</select></label><DepartmentMultiSelect departments={departments} disabled={!canEditInfo} leadDepartmentId={form.leadDepartmentId} selectedIds={form.coordinatingDepartmentIds} onChange={(coordinatingDepartmentIds) => changeDepartments(form.leadDepartmentId, coordinatingDepartmentIds)} />{!isGroup && <><label className="f"><span>Bắt đầu kế hoạch</span><input disabled={!canEditInfo} type="date" value={form.startDate} onChange={(event) => setForm({ ...form, startDate: event.target.value })} /></label><label className="f"><span>Kết thúc kế hoạch</span><input disabled={!canEditInfo} type="date" value={form.endDate} onChange={(event) => setForm({ ...form, endDate: event.target.value })} /></label>{!isNew && <label className="f ro"><span>Kết thúc thực tế</span><input readOnly type="text" value={date(actualCompletionDate(item.actual_completed_at))} /></label>}</>}{!isGroup && canEditInfo && <label className="f wide"><span>Trạng thái</span><select disabled={!canEditInfo} value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as WorkItemStatus })}><option value="not_started">Chưa thực hiện</option><option value="in_progress">Đang thực hiện</option><option value="pending_approval" disabled>Chờ duyệt</option><option value="completed" disabled>Hoàn thành qua duyệt</option></select></label>}{!isGroup && (canEditInfo || canManageParticipants) && <ParticipantMultiSelect users={canEditInfo ? participantUsers : participantUsersForManager} disabled={!canEditInfo && !canManageParticipants} selectedIds={form.participantIds} onChange={(participantIds) => setForm({ ...form, participantIds })} placeholder={form.leadDepartmentId || form.coordinatingDepartmentIds.length ? 'Chọn người thuộc các đơn vị liên quan' : 'Chọn đơn vị chủ trì/phối hợp trước'} />}</div>{participantScope === 'own_department' && <div className="note" style={{ marginTop: 10 }}>Bạn chỉ thay đổi người tham gia thuộc phòng/ban của mình. Phân công của các đơn vị khác được giữ nguyên.</div>}{isGroup && <div className="note" style={{ marginTop: 10 }}>Ngày và trạng thái của hạng mục tự tính từ các công việc con.</div>}{canEditInfo ? <div className="row2" style={{ marginTop: 10 }}><button className="btn pri" disabled={busy || !form.name.trim()}>{busy ? 'Đang lưu…' : isNew ? `Lưu ${isGroup ? 'hạng mục' : 'công việc'}` : 'Lưu thay đổi'}</button>{dirty && !isNew && <span className="tiny unsaved">Có thay đổi chưa lưu</span>}</div> : canManageParticipants && <div className="row2" style={{ marginTop: 10 }}><button type="button" className="btn pri" disabled={busy || !dirty} onClick={() => void saveParticipants()}>{busy ? 'Đang lưu…' : 'Lưu phân công'}</button>{dirty && <span className="tiny unsaved">Có thay đổi phân công chưa lưu</span>}</div>}</form>{(isNew || canAddChild || canManageStructure) && <div className="sec"><h3>Thao tác</h3>{isNew ? <><button className="btn dgr" onClick={() => void requestClose()}>Hủy bỏ, không thêm nữa</button><div className="tiny muted" style={{ marginTop: 8 }}>Bản nháp này chưa được ghi vào database.</div></> : <div className="row2">{canAddChild && <button className="btn" onClick={() => void addChild()}>+ Công việc con</button>}{canManageStructure && <button className="btn dgr" onClick={() => void deleteCurrent()}>Xóa</button>}</div>}</div>}</>}
    {tab === 'evidence' && (!isGroup || item.attachment) && <><div className="sec"><h3>Tài liệu bằng chứng ({item.attachment ? 1 : 0})</h3>{isEvidenceLocked && <div className="note">Bằng chứng đã nộp được khóa để phục vụ xét duyệt và truy vết. Bạn vẫn có thể mở xem nhưng không thể thay hoặc xóa tệp.</div>}{item.attachment ? <div className="evi"><span className="ic">FILE</span><button className="t" onClick={() => void viewEvidence(item.attachment!)}>{item.attachment.file_name}</button><button disabled={!canManageEvidence} title={isEvidenceLocked ? 'Bằng chứng đã được khóa' : undefined} aria-label="Xóa tệp bằng chứng" onClick={() => void deleteEvidence()}>✕</button></div> : canManageEvidence && !isGroup ? <label className="drop">Bấm để chọn tệp bằng chứng<input hidden type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) void run(() => uploadEvidence(item.id, file, profile.id), 'Đã tải lên tệp bằng chứng.') }} /></label> : <div className="tiny muted">Chưa có bằng chứng.</div>}</div>{canManageEvidence && !isGroup && <div className="sec"><h3>Gửi hoàn thành</h3><div className="f"><label><span>{completesDirectly ? 'Ghi chú hoàn thành' : 'Ghi chú gửi duyệt'}</span><textarea value={note} onChange={(event) => setNote(event.target.value)} /></label></div>{isOverdue && <div className="f" style={{ marginTop: 9 }}><label><span>Lý do trễ <b aria-hidden="true">*</b></span><textarea required value={lateReason} onChange={(event) => setLateReason(event.target.value)} placeholder="Bắt buộc nhập vì đã quá ngày kết thúc kế hoạch" /></label></div>}<div className="row2" style={{ marginTop: 9 }}><button className="btn ok" disabled={busy} onClick={() => void submitCompletion()}>{completesDirectly ? 'Nộp bằng chứng và hoàn thành' : 'Gửi quản trị viên duyệt'}</button>{!item.attachment && <span className="tiny muted">Hãy tải lên 1 tệp bằng chứng trước khi gửi.</span>}{item.attachment && isOverdue && !lateReason.trim() && <span className="tiny muted">Công việc quá hạn: cần nhập lý do trễ trước khi gửi.</span>}</div></div>}</>}
    {tab === 'log' && <div className="sec"><h3>Diễn biến công việc ({updates.length})</h3>{updates.length ? <div className="log">{updates.map((update) => <div className="li" key={update.id}><div className="dot" /><div><div className="m">{dateTime(update.created_at)} · {update.author?.full_name || update.author?.username}</div><p>{update.content}</p></div></div>)}</div> : <div className="tiny muted">Chưa có diễn biến nào.</div>}{canLogProgress ? <><div className="f" style={{ marginTop: 10 }}><label><span>Ghi diễn biến mới</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Hiện trường hôm nay thế nào? Vướng gì?" /></label></div><button className="btn pri" disabled={!note.trim() || busy} style={{ marginTop: 8 }} onClick={() => void run(async () => { await addProgress(item.id, note, profile.id); await markWorkItemActivitySeen(item.id, profile.id); setNote(''); setUpdates(await getProgressUpdates(item.id)) }, 'Đã ghi diễn biến mới.')}>Ghi diễn biến</button></> : <div className="note" style={{ marginTop: 10 }}>{progressLockReason}</div>}</div>}
    {tab === 'proposal' && canProposeChild && <WorkProposalForm parent={item} items={items} users={users} departments={departments} onChanged={onChanged} onDirtyChange={setProposalDirty} />}
    {tab === 'approval' && canReviewItem && item.status === 'pending_approval' && <div className="sec approval-panel"><h3>Xét duyệt hoàn thành</h3>{approvalLoading ? <div className="tiny muted">Đang tải yêu cầu chờ duyệt…</div> : pendingRequest ? <><div className="approval-request"><span>Người gửi</span><b>{pendingRequest.submitter?.full_name || pendingRequest.submitter?.username || '—'}</b><span>Thời gian gửi</span><b>{dateTime(pendingRequest.submitted_at)}</b><span>Lần gửi</span><b>{pendingRequest.attempt_no}</b><span>Ghi chú</span><p>{pendingRequest.note || 'Không có ghi chú'}</p>{pendingRequest.late_reason && <><span>Lý do trễ</span><p className="late-reason">{pendingRequest.late_reason}</p></>}</div>{item.attachment && <div className="approval-evidence"><span className="tiny muted">Bằng chứng kèm theo</span><button className="btn" onClick={() => void viewEvidence(item.attachment!)}>Xem {item.attachment.file_name}</button></div>}<label className="f"><span>Nhận xét / lý do từ chối</span><textarea value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} placeholder="Bắt buộc nhập khi từ chối" /></label><div className="approval-panel-actions"><button className="btn dgr" disabled={busy} onClick={() => void decide('rejected')}>{busy ? 'Đang xử lý…' : 'Từ chối'}</button><button className="btn ok" disabled={busy} onClick={() => void decide('approved')}>{busy ? 'Đang xử lý…' : 'Duyệt hoàn thành'}</button></div><div className="tiny muted">Chỉ cần một quản trị viên hợp lệ xử lý; hệ thống không duyệt hai cấp.</div></> : <div className="note warn">Yêu cầu có thể vừa được quản trị viên khác xử lý. Hãy tải lại dữ liệu.</div>}</div>}
...
function TodayLine({ start, pixelsPerDay, label = false }: { start: string; pixelsPerDay: number; label?: boolean }) { const left = dayDiff(start, today()) * pixelsPerDay; const labelAtStart = left < 18; return <>{left >= 0 && <div className="now" style={{ left }} />}{label && left >= 0 && <div className={`nowlab${labelAtStart ? ' edge' : ''}`} style={{ left }}>{new Date().getDate()}/{new Date().getMonth() + 1}</div>}</> }
