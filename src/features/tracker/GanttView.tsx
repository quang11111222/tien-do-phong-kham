...
function WorkDrawer({ item, items, users, departments, profile, isNew, tab, onTabChange, onClose, onCreated, onAddChild, onChanged, onError, onDirtyChange }: { item: WorkItem; items: WorkItem[]; users: UserProfile[]; departments: Department[]; profile: Profile; isNew: boolean; tab: DrawerTab; onTabChange: (tab: DrawerTab) => void; onClose: () => void; onCreated: (id: string) => Promise<void>; onAddChild: (parent: WorkItem) => void; onChanged: () => Promise<void>; onError: (message: string | null) => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  const isLocked = !isNew && (item.status === 'pending_approval' || item.status === 'completed')
  const hasLockedDescendant = !isNew && descendants(item.id, items).some((child) => child.status === 'pending_approval' || child.status === 'completed')
  const canEditInfo = profile.role === 'manager' && !isLocked
  const canManageEvidence = canUpdate && !isLocked
  const canLogProgress = canUpdate && item.status !== 'pending_approval' && item.status !== 'completed'
  const progressLockReason = !canUpdate
    ...
    : item.status === 'pending_approval'
      ? 'Công việc đang chờ duyệt nên tạm khóa cập nhật diễn biến.'
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (isLocked) { onError('Công việc đang chờ duyệt hoặc đã hoàn thành nên không thể chỉnh sửa.'); return }
  ...
  const addChild = async () => { if (isLocked) { onError('Không thể thêm công việc con vào hạng mục/công việc đã khóa.'); return } if (dirty && !await confirm({ title: 'Thêm công việc con?', message: 'Các thay đổi chưa lưu trên panel hiện tại sẽ bị bỏ trước khi mở công việc con mới.', confirmLabel: 'Tiếp tục', tone: 'danger' })) return; onAddChild(item) }
  const deleteCurrent = async () => { if (isLocked || hasLockedDescendant) { onError(hasLockedDescendant ? 'Không thể xóa hạng mục vì bên trong có công việc đang chờ duyệt hoặc đã hoàn thành.' : 'Công việc đang chờ duyệt hoặc đã hoàn thành nên không thể xóa.'); return } if (await confirm({ title: `Xóa ${isGroup ? 'hạng mục' : 'công việc'}?`, message: `${item.name}${hasChildren ? ' và toàn bộ công việc con bên trong' : ''} sẽ bị xóa. Thao tác này không thể hoàn tác.`, confirmLabel: 'Xóa', tone: 'danger' })) await run(() => removeWorkItem(item.id), onClose) }
