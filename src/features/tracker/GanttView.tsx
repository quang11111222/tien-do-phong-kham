...
import { canEditWorkItem, canReviewCompletion } from '../../lib/permissions'
...
export function GanttView({ project, profile, initialWorkItemId, onSelectedWorkItemChange, onBack, onDirtyChange, onUnreadCountChange }: { project: Project; profile: Profile; initialWorkItemId: string | null; onSelectedWorkItemChange: (workItemId: string | null) => void; onBack: () => void; onDirtyChange: (dirty: boolean) => void; onUnreadCountChange: (count: number) => void }) {
  ...
  return <>
    ...
    <div className="status-level-hint"><span><i className="status-shape summary" />Viền màu: trạng thái tổng hợp của hạng mục/công việc cha</span><span><i className="status-shape leaf" />Nền màu: trạng thái của công việc cuối nhánh</span></div>
...
function WorkDrawer({ item, items, users, departments, profile, projectCanManage, isNew, tab, onTabChange, onClose, onCreated, onAddChild, onChanged, onError, onDirtyChange }: { item: WorkItem; items: WorkItem[]; users: UserProfile[]; departments: Department[]; profile: Profile; projectCanManage: boolean; isNew: boolean; tab: DrawerTab; onTabChange: (tab: DrawerTab) => void; onClose: () => void; onCreated: (id: string) => Promise<void>; onAddChild: (parent: WorkItem) => void; onChanged: () => Promise<void>; onError: (message: string | null) => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  const canReviewItem = canReviewCompletion({ profile, projectCanManage, leadDepartmentId: item.lead_department_id })
  ...
  const canUpdate = canEditWorkItem({ role: profile.role, canManageProject: projectCanManage, userId: profile.id, participantIds: item.participant_ids })
  const isEvidenceLocked = !isNew && (item.status === 'pending_approval' || item.status === 'completed')
  const canEditInfo = projectCanManage
