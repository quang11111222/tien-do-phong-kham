...
export function GanttView({ project, profile, initialWorkItemId, onSelectedWorkItemChange, onBack, onDirtyChange, onUnreadCountChange }: { project: Project; profile: Profile; initialWorkItemId: string | null; onSelectedWorkItemChange: (workItemId: string | null) => void; onBack: () => void; onDirtyChange: (dirty: boolean) => void; onUnreadCountChange: (count: number) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (!filterWorkItemsByScope(items, scope, profile.id).some((candidate) => candidate.id === item.id)) setScope('visible')
      setDraftItem(null)
      setSelected(item.id)
  ...
  const scopedItems = useMemo(() => filterWorkItemsByScope(items, scope, profile.id), [items, profile.id, scope])
  ...
  const scopeCounts = {
    mine: allLeaves.filter((item) => matchesWorkScopeDirect(item, 'mine', profile.id)).length,
  ...
  const openDraft = (parent: WorkItem | null) => {
    if ((!parent && !canManageProject) || (parent && !canAddChild(parent))) {
      setError('Bạn chỉ được thêm công việc trong hạng mục do phòng/ban mình chủ trì.')
      return
    }
  ...
  return <>
    ...
    <div className="work-scope"><span>Phạm vi hiển thị</span><div className="seg" role="group" aria-label="Phạm vi công việc"><button className={scope === 'mine' ? 'on' : ''} onClick={() => setScope('mine')}>Việc của tôi <b>{scopeCounts.mine}</b></button><button className={scope === 'visible' ? 'on' : ''} onClick={() => setScope('visible')}>Tất cả công việc liên quan <b>{scopeCounts.visible}</b></button></div><small>{scope === 'mine' ? 'Chỉ hiển thị việc bạn được giao và các hạng mục cha liên quan.' : 'Hiển thị toàn bộ công việc tài khoản này có quyền xem.'}</small></div>
...
function WorkDrawer({ item, items, users, departments, profile, projectCanManage, canManageStructure, canAddChild, isNew, tab, onTabChange, onClose, onCreated, onAddChild, onChanged, onError, onDirtyChange }: { item: WorkItem; items: WorkItem[]; users: UserProfile[]; departments: Department[]; profile: Profile; projectCanManage: boolean; canManageStructure: boolean; canAddChild: boolean; isNew: boolean; tab: DrawerTab; onTabChange: (tab: DrawerTab) => void; onClose: () => void; onCreated: (id: string) => Promise<void>; onAddChild: (parent: WorkItem) => void; onChanged: () => Promise<void>; onError: (message: string | null) => void; onDirtyChange: (dirty: boolean) => void }) {
