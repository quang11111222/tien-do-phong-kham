...
export function GanttView({ project, profile, onBack, onDirtyChange }: { project: Project; profile: Profile; onBack: () => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  return <>
    ...
    {(draftItem ?? selectedItem) && <WorkDrawer key={`${(draftItem ?? selectedItem)!.id}-${(draftItem ?? selectedItem)!.version}-${(draftItem ?? selectedItem)!.attachment?.id ?? ''}`} item={(draftItem ?? selectedItem)!} items={items} users={users} profile={profile} isNew={Boolean(draftItem)} onClose={closeDrawer} onCreated={created} onAddChild={openDraft} onChanged={load} onError={setError} onDirtyChange={onDirtyChange} />}
...
function WorkDrawer({ item, items, users, profile, isNew, onClose, onCreated, onAddChild, onChanged, onError, onDirtyChange }: { item: WorkItem; items: WorkItem[]; users: UserProfile[]; profile: Profile; isNew: boolean; onClose: () => void; onCreated: (id: string) => Promise<void>; onAddChild: (parent: WorkItem) => void; onChanged: () => Promise<void>; onError: (message: string | null) => void; onDirtyChange: (dirty: boolean) => void }) {
