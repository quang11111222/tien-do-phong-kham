...
import { addProgress, cloneProjectPlan, createWorkItem, getDepartments, getMilestones, getProgressUpdates, getProjects, getUsers, getWorkItems, importProjectPlan, markWorkItemActivitySeen, removeEvidence, removeWorkItem, requestCompletion, saveWorkItem, uploadEvidence, viewEvidence } from './trackerService'
...
export function GanttView({ project, profile, onBack, onDirtyChange, onUnreadCountChange }: { project: Project; profile: Profile; onBack: () => void; onDirtyChange: (dirty: boolean) => void; onUnreadCountChange: (count: number) => void }) {
...
function WorkDrawer({ item, items, users, departments, profile, isNew, onClose, onCreated, onAddChild, onChanged, onError, onDirtyChange }: { item: WorkItem; items: WorkItem[]; users: UserProfile[]; departments: Department[]; profile: Profile; isNew: boolean; onClose: () => void; onCreated: (id: string) => Promise<void>; onAddChild: (parent: WorkItem) => void; onChanged: () => Promise<void>; onError: (message: string | null) => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  return <><aside className="drawer on" aria-label={isNew ? `Thêm ${isGroup ? 'hạng mục' : 'công việc'}` : 'Chi tiết công việc'}><div id="dwrap"><div className="dhead"><div style={{ flex: 1 }}><div className="eyebrow">{isGroup ? 'Hạng mục' : 'Công việc'} · {item.wbs}</div><h2>{isNew ? `Thêm ${isGroup ? 'hạng mục' : 'công việc'} mới` : item.name}</h2>{isNew ? <span className="tiny unsaved">Chưa lưu — chỉ nút Lưu mới ghi vào tiến độ</span> : <span className={`pill ${classes[displayedStatus]}`}><i />{displayedStatus === 'late' ? 'Quá hạn' : labels[displayedStatus]}</span>}</div><button className="btn" aria-label="Đóng panel" onClick={() => void requestClose()}>✕</button></div><div className="dtabs"><button className={tab === 'info' ? 'on' : ''} onClick={() => void switchTab('info')}>Thông tin</button>{!isNew && !isGroup && <button className={tab === 'evidence' ? 'on' : ''} onClick={() => void switchTab('evidence')}>Bằng chứng <span className="cnt">{item.attachment ? 1 : 0}</span></button>}{!isNew && <button className={tab === 'log' ? 'on' : ''} onClick={() => void switchTab('log')}>Diễn biến <span className="cnt">{updates.length}</span></button>}</div><div className="dbody">
    {tab === 'info' && <><form className="sec" onSubmit={(event) => void submit(event)}><h3>Thông tin {isGroup ? 'hạng mục' : 'công việc'}</h3><div className="fg"><label className="f wide"><span>{isGroup ? 'Tên hạng mục' : 'Tên công việc'}</span><textarea autoFocus={isNew} disabled={profile.role !== 'manager'} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label className="f wide"><span>Đơn vị chủ trì</span><select disabled={profile.role !== 'manager'} value={form.leadDepartmentId} onChange={(event) => setForm({ ...form, leadDepartmentId: event.target.value, coordinatingDepartmentIds: form.coordinatingDepartmentIds.filter((id) => id !== event.target.value) })}><option value="">Chưa chọn đơn vị chủ trì</option>{departments.map((department) => <option key={department.id} value={department.id}>{department.code} — {department.name}</option>)}</select></label><fieldset className="f wide department-field" disabled={profile.role !== 'manager'}><legend>Đơn vị phối hợp <small>(có thể chọn nhiều)</small></legend><div className="department-options">{departments.map((department) => <label className={department.id === form.leadDepartmentId ? 'disabled' : ''} key={department.id}><input type="checkbox" disabled={department.id === form.leadDepartmentId} checked={form.coordinatingDepartmentIds.includes(department.id)} onChange={(event) => setForm({ ...form, coordinatingDepartmentIds: event.target.checked ? [...form.coordinatingDepartmentIds, department.id] : form.coordinatingDepartmentIds.filter((id) => id !== department.id) })} /><span><b>{department.code}</b><small>{department.name}</small></span></label>)}</div></fieldset>{!isGroup && <><label className="f"><span>Bắt đầu</span><input disabled={profile.role !== 'manager'} type="date" value={form.startDate} onChange={(event) => setForm({ ...form, startDate: event.target.value })} /></label><label className="f"><span>Kết thúc</span><input disabled={profile.role !== 'manager'} type="date" value={form.endDate} onChange={(event) => setForm({ ...form, endDate: event.target.value })} /></label></>}{!isGroup && profile.role === 'manager' && <label className="f wide"><span>Trạng thái</span><select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as WorkItemStatus })}><option value="not_started">Chưa thực hiện</option><option value="in_progress">Đang thực hiện</option><option value="pending_approval" disabled>Chờ duyệt</option><option value="completed" disabled>Hoàn thành qua duyệt</option></select></label>}</div>{isGroup && <div className="note" style={{ marginTop: 10 }}>Ngày của hạng mục tự tính từ các công việc con, không nhập trực tiếp tại đây.</div>}{!isGroup && profile.role === 'manager' && <div className="sec" style={{ marginTop: 10 }}><h3>Người tham gia</h3><div className="ev">{users.map((user) => <label className="evi" key={user.id}><input type="checkbox" checked={form.participantIds.includes(user.id)} onChange={(event) => setForm({ ...form, participantIds: event.target.checked ? [...form.participantIds, user.id] : form.participantIds.filter((id) => id !== user.id) })} /><span><span className="t">{user.full_name}</span><span className="s">{user.role === 'manager' ? 'Quản trị viên' : 'Nhân viên'}</span></span></label>)}</div></div>}{profile.role === 'manager' && <div className="row2" style={{ marginTop: 10 }}><button className="btn pri" disabled={busy || !form.name.trim()}>{busy ? 'Đang lưu…' : isNew ? `Lưu ${isGroup ? 'hạng mục' : 'công việc'}` : 'Lưu thay đổi'}</button>{dirty && !isNew && <span className="tiny unsaved">Có thay đổi chưa lưu</span>}</div>}</form>{profile.role === 'manager' && <div className="sec"><h3>Thao tác</h3>{isNew ? <><button className="btn dgr" onClick={() => void requestClose()}>Hủy bỏ, không thêm nữa</button><div className="tiny muted" style={{ marginTop: 8 }}>Bản nháp này chưa được ghi vào database.</div></> : <div className="row2"><button className="btn" onClick={() => void addChild()}>+ Công việc con</button><button className="btn dgr" onClick={() => void deleteCurrent()}>Xóa</button></div>}</div>}</>}
