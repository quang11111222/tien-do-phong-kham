...
import { getNotificationPolicies, notificationLabels, recipientLabels, saveNotificationPolicies, type NotificationPolicy, type RecipientGroup } from './notificationSettingsService'
    ...
    {loading ? <p>Đang tải cấu hình…</p> : policies.length > 0 && <>
      <fieldset disabled={saving} className="notification-settings-fields">
        {policies.map((policy) => <article className="notification-policy" key={policy.kind}>
          ...
          <div className="notification-recipients">{(Object.keys(recipientLabels) as RecipientGroup[]).filter((group) => policy.kind !== 'assigned' || group === 'participants').map((group) => <label key={group}><input type="checkbox" checked={policy.recipients.includes(group)} onChange={(event) => change(policy.kind, { recipients: event.target.checked ? [...policy.recipients, group] : policy.recipients.filter((value) => value !== group) })} />{recipientLabels[group]}</label>)}</div>
        </article>)}
      </fieldset>
