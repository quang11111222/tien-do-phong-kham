...
import type { Project, UserProfile, WorkItem } from '../../types/domain'
...
export function PortfolioPage({ isManager, isDepartmentAdmin, onOpen }: { isManager: boolean; isDepartmentAdmin: boolean; onOpen: (project: Project) => void }) {
  ...
  useEffect(() => { if (isManager) void getUsers().then(setUsers).catch(() => setError('Không tải được danh sách quản trị dự án.')) }, [isManager])
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      setError(null)
      await saveProject({ ...form, id: form.id || undefined, administratorIds: isManager ? form.administratorIds : undefined })
      setModalOpen(false)
      setForm(emptyForm)
  ...
  return <>
    ...
    <div className="card project-table-wrap">{loading ? <div className="empty">Đang tải dự án…</div> : displayedRows.length ? <table className="ptab"><thead><tr><th>Dự án</th><th>Địa điểm</th><th>Bắt đầu</th><th>Hạn hoàn thành</th><th>{showDeleted ? 'Đã xóa lúc' : 'Còn lại'}</th><th>Tiến độ</th><th>Trễ</th><th /></tr></thead><tbody>{displayedRows.map(({ project, items }) => {
      ...
      const canManage = isManager || project.can_manage
      return <tr className={showDeleted ? 'deleted-project-row' : ''} key={project.id} onClick={() => { if (!showDeleted) onOpen(project) }}><td><span className="pname">{project.name}</span><span className="pcode">{project.code}</span></td><td>{project.site || '—'}</td><td className="num">{date(project.start_date)}</td><td className="num">{project.end_date ? date(project.end_date) : 'chưa chốt'}</td><td className="num">{showDeleted ? dateTime(project.deleted_at) : remaining(project.end_date)}</td><td>{showDeleted ? 'Đang ẩn' : tasks.length ? <><span className="mini"><i style={{ width: `${pct}%` }} /></span>{pct}% · {done}/{tasks.length}</> : 'Chưa lập tiến độ'}</td><td className={late ? 'dl' : ''}>{showDeleted ? '—' : late}</td><td><div className="rowact">{showDeleted && isManager ? <button className="btn sm" disabled={busyProjectId === project.id} onClick={(event) => { event.stopPropagation(); void changeDeleted(project, false) }}>{busyProjectId === project.id ? 'Đang xử lý…' : 'Khôi phục'}</button> : canManage && !showDeleted ? <><button className="btn sm" onClick={(event) => { event.stopPropagation(); setForm({ id: project.id, code: project.code, name: project.name, site: project.site ?? '', startDate: project.start_date ?? '', endDate: project.end_date ?? '', administratorIds: project.administrator_ids ?? [] }); setModalOpen(true) }}>Sửa</button><button className="btn dgr sm" disabled={busyProjectId === project.id} onClick={(event) => { event.stopPropagation(); void changeDeleted(project, true) }}>{busyProjectId === project.id ? 'Đang xóa…' : 'Xóa'}</button></> : null}</div></td></tr>
    })}</tbody></table> : <div className="empty"><h3>{showDeleted ? 'Chưa có dự án đã xóa' : 'Chưa có dự án'}</h3><p>{showDeleted ? 'Dự án xóa mềm sẽ xuất hiện tại đây để quản trị viên có thể khôi phục.' : 'Quản trị viên có thể thêm dự án mới để bắt đầu lập tiến độ.'}</p></div>}</div>
    {isManager && <div className="note" style={{ marginTop: 12 }}><b>Xóa dự án</b> là xóa mềm: dự án bị ẩn khỏi người dùng nhưng dữ liệu tiến độ, bằng chứng và nhật ký không bị mất. Quản trị viên có thể khôi phục trong mục <b>Đã xóa</b>.</div>}
    <div className={`modal ${modalOpen ? 'on' : ''}`}><form className="mbox" onSubmit={(event) => void submit(event)}><header><h2>{form.id ? 'Sửa dự án' : 'Thêm dự án'}</h2><button type="button" className="btn" onClick={() => setModalOpen(false)}>✕</button></header><div className="mbody"><div className="fg"><label className="f"><span>Mã dự án</span><input required value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} /></label><label className="f"><span>Tên dự án</span><input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label className="f wide"><span>Địa điểm</span><input value={form.site} onChange={(event) => setForm({ ...form, site: event.target.value })} /></label><label className="f"><span>Bắt đầu</span><input type="date" value={form.startDate} onChange={(event) => setForm({ ...form, startDate: event.target.value })} /></label><label className="f"><span>Hạn hoàn thành</span><input type="date" min={form.startDate || undefined} value={form.endDate} onChange={(event) => setForm({ ...form, endDate: event.target.value })} /></label>{isManager && <ParticipantMultiSelect users={users.filter((user) => user.username !== 'admin')} selectedIds={form.administratorIds} onChange={(administratorIds) => setForm({ ...form, administratorIds })} label="Quản trị dự án" placeholder="Chọn quản trị dự án" />}</div></div><footer className="mfoot"><button type="button" className="btn" onClick={() => setModalOpen(false)}>Hủy</button><button className="btn pri">Lưu dự án</button></footer></form></div>
  </>
}
